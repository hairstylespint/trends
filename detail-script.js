document.addEventListener('DOMContentLoaded', function() {     
    ...
    function runAGC(keyword) {         
        ...
        function generateSeoTitle(baseKeyword) {              
            const hookWords = ['Stunning', 'Trendy', 'Elegant', 'Chic', 'Beautiful', 'Modern', 'Classic', 'Gorgeous', 'Best', 'Amazing'];              
            const suffixWords = ['Hairstyle', 'Haircut', 'Style Guide', 'Ideas', 'Looks'];             
        ...
        function fallbackDescription(term) {             
            const spintaxArticleTemplate = `{Discover|Explore} the best <strong>${capitalizeEachWord(term)}</strong> {hairstyles|styling ideas} to {instantly elevate|perfectly transform} your {look|appearance}.`;             
        ...
        function populateMainContent(term) {             
            ...
            document.title = `${newTitle} | LuxeHair`;             
            if(detailTitle) detailTitle.textContent = newTitle;             
            const queryImage = term + " hairstyle hair beauty";             
            ...
            fetch('keyword.txt')                 
                ...
                .then(data => {                     
                    ...
                    selectedKeywords.forEach(relatedTerm => {                         
                        ...
                        const queryImage = relatedTerm + " hairstyle hair model";                         
        ...
        window.handleRelatedSuggest = function(data) {             
            ...
            if (suggestions && suggestions.length > 0) {                 
                suggestions.forEach(item => {                     
                    ...
                    let cleanTerm = relatedTerm ? relatedTerm.replace(/hairstyle/gi, '').trim() : '';                     
                    ...
                    const linkUrl = `detail.html?q=${encodeURIComponent(keywordForUrl)}`;                                          
                    const queryImage = cleanTerm + " hairstyle hair model";                     
                    const imageUrl = `https://tse1.mm.bing.net/th?q=${encodeURIComponent(queryImage)}&w=400&h=400&c=7&rs=1&p=0&dpr=1.5&pid=1.7`;                                          
